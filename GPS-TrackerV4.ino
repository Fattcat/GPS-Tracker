...
void bootScreen() {
  ...
  display.setCursor(15, 4);  display.println(F("GPS"));
  display.setCursor(24, 24); display.println(F("TRACKER"));
  display.setTextSize(1);
  display.setCursor(70, 7); display.println(F("v4.1"));
  display.setCursor(10, 50); display.println(F("Inicializujem..."));
  display.display();
  delay(2000);
}
...
// Globálna premenná pre OLED
int lastSats = -1;  // nikdy nevalidná hodnota, aby sa OLED napísalo prvýkrát
// Globálne pre blinkovanie satelitov
bool blinkState = true;
unsigned long lastBlinkMs = 0;
const unsigned long BLINK_INTERVAL = 250; // 250ms
...
void refreshDisplay(unsigned long now) {
  ...
  bool moving = (spd >= MIN_SPEED_KMH);

  // -------------------------
  // Automatický UTC offset pre SK (zimný/letný čas)
  // -------------------------
  int utcOffset = 1; // predpoklad: zimný čas
  if (gps.date.isValid()) {
    int y = gps.date.year();
    int m = gps.date.month();
    int d = gps.date.day();
    // posledná nedeľa marca = letný čas
    if (m == 3) {
      int lastSun = 31 - (int)((5*y/4 + 4) % 7);
      if (d >= lastSun) utcOffset = 2;
    }
    // posledná nedeľa októbra = zimný čas
    if (m == 10) {
      int lastSun = 31 - (int)((5*y/4 + 1) % 7);
      if (d < lastSun) utcOffset = 2;
    }
  }
  ...
  // ČAS
  // =========================
  int h = gps.time.isValid() ? ((int)gps.time.hour() + UTC_OFFSET) % 24 : 0;
  int min = gps.time.isValid() ? (int)gps.time.minute() : 0;
  display.setTextSize(3);
  display.setCursor(30, 0);
  display.printf("%02d:%02d", h, min);
  ...
  // =========================
  // SATELITY (ľavý horný roh) — aktualizuj len ak sa zmenili
  // =========================
  if (sats < 4) {
    ...
    if (blinkState) {
      ...
      display.print("      "); // vymaže S:XX počas vypnutia blinku
    }
  } else {
    // 4 a viac satelitov → stále svieti
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.print("S:");
    display.print(sats);
  ...
  // =========================
  // POHYB / STOJÍ (pravý horný roh)
  // =========================
  display.setCursor(80, 25);
  display.print(moving ? "IDES" : "STOJIS");

  // =========================
  // NADMORSKÁ VÝŠKA (ľavý spodok)
  // =========================
  ...
  if (totalDistM < 1000.0f) {
    display.print((int)totalDistM);
    display.print("m");
  } else {
    display.print(totalDistM / 1000.0f, 2);
    display.print("km");
  }

  // =========================
  // RÝCHLOSŤ (pravý spodok) s rezervou pre 3-miestnu hodnotu
  // =========================
  display.setCursor(0, 40);
  if (spd >= 1.0f) {
    display.printf("Rychlost:%5.1f", spd); // 5 znakov rezervy
  } else {
    display.print("R:0.0");
  }
  display.print("km/h");
