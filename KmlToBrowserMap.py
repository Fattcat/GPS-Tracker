...
/* ── INFO PANEL ── */
#info{{
  position:fixed; top:calc(var(--tb)+10px); right:12px; z-index:1000;
...
/* ── 3D OVERLAYS ── */
#hud3{{
  position:fixed; top:calc(var(--tb)+10px); left:12px; z-index:1000;
  background:rgba(11,14,20,.85); border:1px solid var(--border);
  border-radius:var(--r); padding:10px 14px; font-size:10px; color:var(--muted);
  pointer-events:none; display:none; line-height:1.9;
}}
#hud3 b{{ color:var(--accent); font-size:11px; display:block; margin-bottom:2px; }}
#leg3{{
  position:fixed; bottom:calc(var(--ch)+12px); right:12px; z-index:1000;
  background:rgba(11,14,20,.85); border:1px solid var(--border);
  border-radius:var(--r); padding:9px 13px; font-size:10px; color:var(--muted);
  display:none;
}}
#gbar{{
...
function init3D() {{
  ...
  const S=180/hR, AE=Math.max(1,hR/altRange*.3);
  T3._d={{xA,zA,yA,S,AE}};
...
/* ── INFO PANEL ── */
#info{{
  position:fixed; top:calc(var(--tb)+10px); right:200px; z-index:1000;
  background:rgba(11,14,20,.93); border:1px solid var(--border);
  border-radius:var(--r); padding:12px 15px; min-width:200px;
  backdrop-filter:blur(10px); font-size:11px;
  box-shadow:0 6px 28px rgba(0,0,0,.5);
